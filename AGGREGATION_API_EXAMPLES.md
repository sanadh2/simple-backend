...
async function testAnalytics() {
	...
	const trends = await fetch(
		"http://localhost:3000/api/analytics/registration-trends?days=7",
		{
			headers: { Authorization: `Bearer ${accessToken}` },
		}
	).then((r) => r.json())
	...
	const search = await fetch(
		"http://localhost:3000/api/analytics/search?q=john",
		{
			headers: { Authorization: `Bearer ${accessToken}` },
		}
	).then((r) => r.json())
	...
	const users = await fetch(
		"http://localhost:3000/api/analytics/users?page=1&limit=10",
		{
			headers: { Authorization: `Bearer ${accessToken}` },
		}
	).then((r) => r.json())
	...
	const domains = await fetch(
		"http://localhost:3000/api/analytics/email-domains",
		{
			headers: { Authorization: `Bearer ${accessToken}` },
		}
	).then((r) => r.json())
