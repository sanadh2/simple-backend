...
function createRetryableError(message: string, retryAfter?: number): RetryableError {
...
export const bookmarkWorker = new Worker<BookmarkTagJob>(
  ...
  async (job: Job<BookmarkTagJob>) => {
    ...
    try {
      ...
      if (!aiResult.tags || aiResult.tags.length === 0 || aiResult.tags.includes('uncategorized')) {
        if (attemptNumber < 3) {
      ...
      if (!isRetryable) {
        const permanentError = new Error(`Permanent failure: ${errorMessage}`) as RetryableError;
      ...
      if (error instanceof Error && 'retryAfter' in error) {
        ...
        if (retryError.retryAfter) {
          ...
          throw new Error(`Retrying after ${retryError.retryAfter}ms: ${errorMessage}`);
        }
      }
  ...
  if (isPermanentFailure && job) {
    (async () => {
      try {
        ...
        if (bookmark && (!bookmark.tags || bookmark.tags.length === 0 || bookmark.tags.includes('uncategorized'))) {
