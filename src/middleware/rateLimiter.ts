...
function createSendCommand() {
  return async (command: string, ...args: (string | number)[]): Promise<RedisReply> => {
...
export const globalLimiter = rateLimit({
  ...
  message: {
    success: false,
    message: 'Too many requests from this IP (max 1000 per 15 min), please try again later',
  },
  skip: (_req) => {
...
export const authLimiter = rateLimit({
  ...
  message: {
    success: false,
    message: 'Too many authentication attempts (max 20 per 15 min), please try again later',
  },
...
export const apiLimiter = rateLimit({
  ...
  message: {
    success: false,
    message: 'Too many API requests (max 200 per 15 min), please try again later',
  },
  skip: (_req) => {
    return env.NODE_ENV === 'test' || env.NODE_ENV === 'development';
  },
