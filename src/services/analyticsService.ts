...
export class AnalyticsService {
  ...
  static async getRegistrationTrends(days: number = 30): Promise<RegistrationTrend[]> {
  ...
  static async searchUsersByName(searchTerm: string, limit: number = 20): Promise<SearchResult[]> {
    return await User.aggregate([
      {
        $addFields: {
          fullName: { $concat: ['$firstName', ' ', '$lastName'] }
  ...
  static async getUsersWithPagination(page: number = 1, limit: number = 10): Promise<PaginatedUsers> {
    ...
    const data = result[0] as { users: SearchResult[]; metadata?: { totalCount: number; totalPages: number } } | undefined;
    return {
      users: data?.users || [],
  ...
  static async getInactiveUsers(daysInactive: number = 30): Promise<InactiveUser[]> {
