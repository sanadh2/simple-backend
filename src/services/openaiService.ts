	...
	private static async callOllama(
		...
		try {
			...
			if (!data || !data.message) {
				...
				throw new Error(
					`Invalid response format from Ollama. Expected message, got: ${JSON.stringify(data)}`
				)
			}
				...
				[
					{
						role: "system",
						content:
							"You are a helpful assistant that categorizes bookmarks and generates relevant tags. Always respond with ONLY valid JSON, no additional text or explanation.",
					},
					{
						role: "user",
						content,
			...
			const response = await this.callOllama([
				{
					role: "system",
					content:
						"You are a helpful assistant that creates brief, informative descriptions for bookmarks.",
				},
				{
					role: "user",
