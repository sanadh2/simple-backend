...
export class LogService {
	static async getLogs(
		...
		if (filters.startDate || filters.endDate) {
			...
			if (filters.startDate) {
				;(matchStage.timestamp as Record<string, unknown>).$gte =
					filters.startDate
			}
			if (filters.endDate) {
				;(matchStage.timestamp as Record<string, unknown>).$lte =
					filters.endDate
			}
		}
	...
	static async getRecentErrors(limit: number = 20): Promise<
		Array<{
			timestamp: Date
			level: string
			correlationId: string
	...
	static async getLogTrends(days: number = 7): Promise<
		Array<{
			date: string
